...
<h5 class="modal-title" id="postModalLabel">Post to all</h5>
...
@csrf
    ...
    console.log("Bootstrap Select Loaded:", typeof $.fn.selectpicker !== 'undefined');
</script>
    <div class="mb-3">
        <label for="user">User</label>
        <select class="form-control selectpicker" id="user" name="share_user_ids[]" multiple data-live-search="true" data-actions-box="true">
    ...
    $(document).ready(function() {
        console.log("jQuery Loaded:", typeof jQuery !== 'undefined'); // Debugging step
        console.log("Bootstrap Select Loaded:", typeof $.fn.selectpicker !== 'undefined'); // Debugging step

        if ($.fn.selectpicker) {
            $('.selectpicker').selectpicker();
        } else {
            console.error("Bootstrap Select failed to load.");
        }
    });
</script>
    </div>
    ...
    <div class="modal-icons mt-3">
        ...
        <div class="d-flex align-items-center mb-2">
            ...
            <div id="previewContainer" style="margin-top: 1px;" class="preview-area"></div>
        </div>
        ...
        <div class="d-flex align-items-center mb-2">
            ...
            <input id="documentUpload" type="file" name="pdf" accept=".pdf,.doc,.docx,.txt" multiple>
        </div>
        <!-- Add Video Section -->
        <div class="d-flex align-items-center mb-2">
            ...
            <input id="videoUpload" name="video" type="file" accept="video/*" multiple>
        </div>
    </div>
</div>
...
<div class="dropdown">
    ...
    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton">
            ...
            <!-- Hide/Show Option -->
            <li>
                <form action="{{ route('vlogs.toggleStatus', $vlog->id) }}" method="POST">
                    @csrf
                    <button type="submit" class="dropdown-item">
                        {{ $vlog->status ? 'Show this post' : 'Hide this post' }}
                    </button>
                </form>
            </li>
            ...
            <!-- Delete Option -->
           <!--  <li>
                <a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#deleteVlogModal" 
                   onclick="setDeleteForm('{{ route('vlogs.destroy', $vlog->id) }}')">
                    Delete this post
                </a>
            </li> -->
...
@if(!empty($vlog->mentioned_users_data) && !$vlog->mentioned_users_data->isEmpty())

<div>
@foreach($vlog->mentioned_users_data as $user)
<p><h6>@ {{ $user->name }} ({{ $user->designation }})</h6></p>
@endforeach
</div>
@endif
@if($vlog->image)
    <img src="{{ asset('public/' . $vlog->image) }}" style="height:300px;width:600px;" class="img-fluid" alt="Vlog Image">
@endif

@if($vlog->pdf)
    <a href="{{ asset('public/' . $vlog->pdf) }}" target="_blank">View PDF</a>
@endif

@if($vlog->video)
    <video width="320" controls>
        <source src="{{ asset('public/' . $vlog->video) }}" type="video/mp4">
...
$('#response').html('<p style="color: red;">Submission failed</p>');
}
},
error: function(response) {
$('#response').html('<p style="color: red;">Error occurred</p>');
}
});
});
});
...
$(document).ready(function () {
    // Dynamic File Preview for Each Input
    $('input[type="file"]').on('change', function (e) {
        const file = e.target.files[0];
        const previewContainer = $('#previewContainer'); // Correct selector for preview area
        ...
        if (file) {
            ...
            if (fileType.startsWith('image/')) {
                const imgPreview = `<img src="${URL.createObjectURL(file)}" 
                                    style="max-width: 200px; border: 2px solid #ddd; border-radius: 8px; padding: 5px;" />`;
                ...
                previewContainer.html(pdfIcon);

            }else {
                previewContainer.html('<p style="color: red;">Unsupported file format.</p>');
            }
        }
    });
});
